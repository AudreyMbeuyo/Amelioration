        ...
        <div class="mt-4">
            <div class="bg-white shadow overflow-hidden sm:rounded-lg">
                <div class="px-4 py-5 sm:px-6 flex justify-between items-center">
                    ...
                    <button type="button" onclick="openEnseignantModal()" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-orange hover:bg-orange-dark">
                        <svg class="-ml-1 mr-2 h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                        </svg>
                        Ajouter
        ...
        <div class="mt-3">
            ...
            <form action="{{ route('matieres.store') }}" method="POST" class="mt-4">
                ...
                <div class="form-group">
                    <label class="block text-gray-700 text-sm font-bold mb-2" for="enseignant_id">
                        Enseignant
                    </label>                    
                ...
                <div class="mb-4">
                    ...
                    <select name="classe_id[]" id="classe_id" multiple
                            class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-orange">
                        @foreach($classes as $classe)
                            <option value="{{ $classe->id }}">{{ $classe->nom }}</option>
                        @endforeach
                    </select>
                    <p class="text-sm text-gray-500 mt-1">Maintenez Ctrl (ou Cmd sur Mac) pour sélectionner plusieurs classes.</p>
                </div>
...
<div id="enseignantModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full" style="z-index: 999;">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3">
            <h3 class="text-lg font-medium text-navy">Ajouter un enseignant</h3>
            <form action="{{ route('enseignants.store') }}" method="POST" class="mt-4">
                @csrf
                <input type="hidden" name="departement_id" value="{{ $departement->id }}">
                <div class="mb-4">
                ...
                <div class="flex justify-end space-x-3">
                    <button type="button" onclick="document.getElementById('enseignantModal').classList.add('hidden')"
                            class="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300">
                        Annuler
                    </button>
                    <button type="submit"
