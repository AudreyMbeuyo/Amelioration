...
export default function AdminDepartments() {
    ...
    const BACKEND_URL = process.env.NEXT_PUBLIC_BACKEND_URL || 'http://agence-voyage.ddns.net:9026/api';
            ...
            const mockDepartments = [
                {
                    ...
                    nom: 'Informatique',
                    code: 'INFO',
                    description: 'Département des Sciences Informatiques et Technologies',
                    chef: {
                        id: 5,
                ...
                },
                {
                    ...
                    nom: 'Physique',
                    code: 'PHYS',
                    description: 'Département de Physique Appliquée',
                    chef: null,
                    totalEnseignants: 6,
                    totalEtudiants: 240,
                    dateCreation: '2023-09-01',
    ...
    const handleDeleteDepartment = async () => {
        ...
        try {
            ...
            console.log('Suppression du département:', selectedDepartment.id);

            setDepartments(prev => prev.filter(dept => dept.id !== selectedDepartment.id));
            setSuccessMessage(`Département "${selectedDepartment.nom}" supprimé avec succès`);
    ...
    const filteredDepartments = departments.filter(dept =>
        ...
        dept.chef?.nom.toLowerCase().includes(searchTerm.toLowerCase())
    );
                                            ...
                                            <div className="ml-4 flex-1 min-w-0">
                                                <div className="flex items-center">
                                                    ...
                                                    <span className={`ml-2 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                                                        department.statut === 'active'
                                                            ? 'bg-green-100 text-green-800'
                                                            : 'bg-gray-100 text-gray-800'
                                                    }`}>
                            {department.statut === 'active' ? 'Actif' : 'Inactif'}
                          </span>
                                                </div>
                                                ...
                                                <div className="mt-2 flex items-center text-sm text-gray-500">
                                                    ...
                                                    {department.chef ? (
                                                        <span>Chef: {department.chef.nom}</span>
                                                    ) : (
